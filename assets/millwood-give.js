  	...
  	this.fn = {
  		...
  		'stripe_utils' : {
			...
			'get_payment_intents' : function (amount) {
				...
				$.ajax({
					'url': millwood.vars.rest_url + 'stripe/v1/create_intent',
					'type': 'POST',
							...
							if (nextele == 'form-wrapper') {
								...
								if (_this.fn.stripe_utils.check_userinfo() != false) {
									...
									if (nextele == 'confirm-section') {
										...
										$(_this.stripe.stripeWrapper).find('.confirm-wrapper').find('#zip').html(_this.stripe.ownerInfo.owner.address.zip)
